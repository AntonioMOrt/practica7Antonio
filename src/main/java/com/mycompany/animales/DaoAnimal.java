...
public class DaoAnimal {
  ...
  public List<Animal> consulta() {
     ...
     * clase "Pasatiempo". */
    return em.createQuery("SELECT c FROM Animal c ORDER BY c.nombre",
        Animal.class).getResultList();
  }
  public Animal busca(Integer id) {
    return em.find(Animal.class, id);
  }
  public void agrega(Animal modelo) {
    em.persist(modelo); // Agrega el modelo a la base de datos.
  }
  public void modifica(Animal modelo) {
    em.merge(modelo);// Guarda los cambios al modelo.
  }
  public void elimina(Animal modelo) {
    // Busca el modelo en base a su id.
    final Animal anterior = em.find(Animal.class, modelo.getId());
